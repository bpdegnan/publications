function result=runme_min
flag_plot=0;

load nfetdata.mat
a_vds=[1 2 3];
a_excludelist{1} = [24:length(nfet)] ;  %this is the index exclude for bad data for set 1, etc
a_excludelist{2} = [13 22:31 37] ;
a_excludelist{3} = [8 24:30 37] ;
...
%             figure
%         end
        h_fet=nfet(i).min.sweep(i_vds);
        h_fet_t=nfet(i).min.tread;
